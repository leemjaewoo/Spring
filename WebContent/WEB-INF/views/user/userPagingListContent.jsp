		...
		<tbody>
			<c:forEach items="${userList }" var="vo">
				<tr class="userTr" data-userid="${vo.userId }">
					...
					<td>-</td>
					<!-- 생략 -->
					<td><fmt:formatDate value="${vo.reg_dt}" pattern="yyyy/MM/dd"></fmt:formatDate></td>
					<!-- reg_dt -->
				</tr>
			</c:forEach>
		</tbody>
	...
	<nav style="text-align: center;">
		<ul class="pagination">
			<c:choose>
				...
				<c:otherwise>
					<li><a href="${cp }/user/userPagingList" aria-label="Previous">
			...
			<c:forEach begin="1" end="${lastPage }" var="i">
				...
				<li class="${active }"><a
					href="${cp }/user/userPagingList?page=${i}">${i}</a></li>
			</c:forEach>
			...
			<c:choose>
				...
				<c:otherwise>
					<li><a
						href="${cp }/user/userPagingList?page=
												${lastPage}"
